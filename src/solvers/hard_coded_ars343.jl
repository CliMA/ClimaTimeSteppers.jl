function step_u!(integrator, cache::IMEXARKCache, ::ARS343)
    ...
    if !isnothing(newtons_method_cache)
        ...
        if (!isnothing(jacobian)) && needs_update!(newtons_method.update_j, NewTimeStep(t))
            NVTX.@range "Wfact" color = colorant"yellow" begin
    ...
    NVTX.@range "Stage 1" color = colorant"yellow" begin
        i::Int = 1
        t_exp = t
        ...
        i = 2
        t_exp = t + dt * c_exp[i]
        NVTX.@range "U=u+dt*a_exp*T_lim" color = colorant"yellow" begin
    ...
    NVTX.@range "Stage 2" color = colorant"yellow" begin
        ...
        i = 3
        t_exp = t + dt * c_exp[i]
        NVTX.@range "U=u+dt*..." color = colorant"yellow" begin
    ...
    NVTX.@range "Stage 3" color = colorant"yellow" begin
        NVTX.@range "temp = U[i]" color = colorant"yellow" begin
            @. temp = U[i] # used in closures
        end
        let i = i
        ...
        NVTX.@range "U+=dt*..." color = colorant"yellow" begin
            @. U[i] +=
    ...
    NVTX.@range "Stage 4" color = colorant"yellow" begin
        NVTX.@range "temp = U[i]" color = colorant"yellow" begin
            @. temp = U[i] # used in closures
        end
        let i = i
        ...
        t_final = t + dt
        NVTX.@range "temp=u+dt*..." color = colorant"yellow" begin
