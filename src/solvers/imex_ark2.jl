...
function cache(prob::DiffEqBase.AbstractODEProblem, alg::NewIMEXARKAlgorithm; kwargs...)
    ...
    U = SparseContainer(ntuple(i->similar(u0), length(inds)), inds)
    T_lim = SparseContainer(ntuple(i->similar(u0), length(inds_T_exp)), inds_T_exp)
    T_exp = SparseContainer(ntuple(i->similar(u0), length(inds_T_exp)), inds_T_exp)
    T_imp = SparseContainer(ntuple(i->similar(u0), length(inds_T_imp)), inds_T_imp)
    temp = similar(u0)
