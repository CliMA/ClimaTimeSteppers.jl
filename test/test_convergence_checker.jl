...
@testset "ConvergenceChecker" begin
    ...
    function test_func(checker, last_iter1, last_iter2)
        ...
        for (err_func, last_iter) in (
            (err_func1, last_iter1),
            (err_func2, last_iter2),
        )
            for iter in 0:(last_iter - 1)
                run!(checker, cache, val_func(iter), err_func(iter), iter) &&
                    return false
            end
            run!(
                checker,
    ...
    for (condition, last_iter1, last_iter2) in
        zip(conditions, last_iters1, last_iters2)
        norm_checker = ConvergenceChecker(; norm_condition = condition)
        component_checker =
            ConvergenceChecker(; component_condition = condition)
        for checker in (norm_checker, component_checker)
            @test test_func(checker, last_iter1, last_iter2)
        end
    end
    for (condition_combiner, last_iter_combiner) in
        ((all, maximum), (any, minimum))
