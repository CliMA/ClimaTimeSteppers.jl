...
abstract type RosSSPScalarAlgorithmName <: ClimaTimeSteppers.AbstractAlgorithmName end

"""
    RosenbrockTableau

Contains everything that defines a RosSSPScalar method.

Refer to the documentation for the precise meaning of the symbols below.
"""
struct RosSSPScalarTableau
    η
    ΘI
    ΘE
    αI
...
"""
struct RosSSPScalarAlgorithm{T <: RosSSPScalarTableau} <: ClimaTimeSteppers.DistributedODEAlgorithm
    tableau::T
end

struct DaiYagiScalar <: RosSSPScalarAlgorithmName end

function tableau(::DaiYagiScalar)
...
function ClimaTimeSteppers.init_cache(prob, alg::RosSSPScalarAlgorithm; kwargs...)
...
function ClimaTimeSteppers.step_u!(int, cache::RosSSPScalarCache)
    (; u, p, t, dt) = int
    f = int.sol.prob.f
