...
The families differ in where the fast and slow components exchange information, not primarily in how often.
...
Choose stage exchange when fast and slow accuracy are coupled through the stages; choose step exchange when the slow tendency should be amortized across full-step fast sub-cycling or when the composition needs an outer implicit complement.
