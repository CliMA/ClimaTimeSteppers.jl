...
const kilometers = kilometer = 1.0
...
# The diffusion equation for $u$ is $\partial_t u = K \Nabla^2 u$, with $K$
...
# Notice how we picked `WeakDivergence` instead of normal divergence.
# TODO: Explain why
...
function actual_T_imp!(∂ₜY, Y, _, _)
...
# TODO: This next section needs to be written by someone who knows how things
# work

# Defining `Wfact` for ClimaCore simulations is a little involved. First, we
# need to define a new type, which we call `_ImplicitJacobian`. This type
# contains a representation of the Jacobian and a solver from ClimaCore to solve
# the resulting system of linear equations.

struct _ImplicitJacobian{M, S}
    matrix::M
    solver::S
end

# Alongside the struct, we need to provide a new method to `LinearAlgebra.ldiv!`
# that lowers to the ClimaCore solver. This will almost always be like this:

# Using `a` to solve `x = a \ b`
function LinearAlgebra.ldiv!(x, a::_ImplicitJacobian, b)
    ClimaCore.MatrixFields.field_matrix_solve!(a.solver, x, a.matrix, b)
    return nothing
end

# It is also convenient to specify a new method for similar, which some solvers
# use internally.
Base.similar(implicitjac::_ImplicitJacobian) = implicitjac

# Finally, we provide a constructor that takes the ClimaCore objects and returns
# the _ImplicitJacobian
function ImplicitJacobian(matrix, Y; alg = ClimaCore.MatrixFields.BlockDiagonalSolve())
    return _ImplicitJacobian(matrix, ClimaCore.MatrixFields.FieldMatrixSolver(alg, matrix, Y))
end
...
# TODO: Why are things the way the are?
...
T_imp! = SciMLBase.ODEFunction(actual_T_imp!; jac_prototype = ImplicitJacobian(jacobian_matrix, Y₀), Wfact = Wfact)
...
prob = SciMLBase.ODEProblem(ClimaTimeSteppers.ClimaODEFunction(; T_imp!, T_exp!, dss!), Y₀, (t0, t_end), nothing)
# prob = SciMLBase.ODEProblem(ClimaTimeSteppers.ClimaODEFunction(; T_exp!, dss!), Y₀, (t0, t_end), nothing)

# We use SSPKnoth for this example
algo = ClimaTimeSteppers.RosenbrockAlgorithm(ClimaTimeSteppers.tableau(ClimaTimeSteppers.SSPKnoth(), Float64))
