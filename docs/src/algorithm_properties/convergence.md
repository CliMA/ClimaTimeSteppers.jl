...
In `ClimaTimeSteppers.jl`, algorithms are rigorously verified during CI to ensure they achieve their theoretical convergence order $p$ under both explicit and implicit integration dynamics. This includes the IMEX ARK and IMEX SSPRK families as well as the explicit-only ERK methods (`SSP22Heuns`, `SSP33ShuOsher`, `RK4`) and the Rosenbrock method (`SSPKnoth`), each tested on the subset of test cases that match their tendency structure.
