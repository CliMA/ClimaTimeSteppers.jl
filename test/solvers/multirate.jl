...
@testset "Multirate convergence" begin
    ...
    # Only Wicker-Skamarock methods converge correctly on both problems.
    passing_algs_orders = [
    ...
    # TODO: These multirate algorithms show convergence orders ≈ 0 on both
    # test problems (errors don't decrease with dt refinement).
    broken_algs_orders = [
    ...
    for (name, prob, sol) in names_probs_sols
        for (alg, expected_order) in broken_algs_orders
            computed_order = convergence_order(prob, sol, alg, dts; fast_dt = 0.5^12)
            @test_broken abs(computed_order - expected_order) / expected_order < 0.15
        end
    end
end
