...
function predicted_convergence_order(algorithm_name::AbstractAlgorithmName, ode_function::AbstractClimaODEFunction)
    ...
    has_exp = !(isnothing(ode_function.T_exp!) && isnothing(ode_function.T_exp_T_lim!))
...
function verify_convergence(
    ...
    numerical_reference_num_steps = nothing,
    ...
    ref_sol = if isnothing(numerical_reference_algorithm_name)
        ...
        @assert !isnothing(numerical_reference_num_steps)
    ...
    plot1_dts = t_end ./ round.(Int, num_steps .* num_steps_scaling_factor .^ range(-1, 1, num_samples))
    plot1 = Plots.plot(;
    ...
    for algorithm_name in algorithm_names
        ...
        test_indices = predicted_order < 4 ? ((num_samples - num_test_points + 1):num_samples) : (1:num_test_points)
        order, order_uncertainty =
            convergence_order(plot1_dts[test_indices], plot1_net_avg_errs[test_indices], order_confidence_percent / 100)
...
function limiter_summary(::Type{FT}, algorithm_names, test_case_type, num_steps) where {FT}
    to_title(name) = replace(string(name), '_' => ' ')
    table_rows = []
    example_test_case = test_case_type(FT)
    test_name = replace(lowercase(example_test_case.test_name), ' ' => '_')
    tracer_names = propertynames(example_test_case.split_prob.u0.ρq)
    n_tracers = length(tracer_names)
    if length(ARGS) == 1
        mkpath("output_$(ARGS[1])")
    else
        mkpath("output")
    end
    for algorithm_name in algorithm_names
        ...
        for use_limiter in (false, true), use_hyperdiffusion in (false, true)
            ...
            algorithm =
                algorithm_name isa ClimaTimeSteppers.ARKAlgorithmName ? ARKAlgorithm(algorithm_name, NewtonsMethod()) :
                RKAlgorithm(algorithm_name)
            solution = @time solve(deepcopy(prob), algorithm; dt).u
            initial_q = solution[1].ρq ./ solution[1].ρ
            final_q = solution[end].ρq ./ solution[end].ρ

            if isempty(plots)
                for name in tracer_names
                    initial_q_plot = Plots.plot(initial_q.:($name); plot_kwargs..., title = to_title(name))
                    push!(plots, initial_q_plot)
                end
            end
            for name in tracer_names
                final_q_plot = Plots.plot(final_q.:($name); plot_kwargs..., title = "")
                push!(plots, final_q_plot)
            end
            ...
            for name in tracer_names
                ...
                if Δϕ₀ == 0
                    Δϕ₀ = 1 # Modify Δϕ₀ to avoid divisions by 0.
                end
                ϕ_error = ϕ .- ϕ₀
                table_row = [
                    alg_str;;
        ...
        plot = Plots.plot(
            plots...,
            colorbar_plot;
            layout = (Plots.@layout [Plots.grid(5, n_tracers) a{0.1w}]),
        ...
        Plots.savefig(plot, joinpath("output", "$(test_name)_limiter_summary_$alg_str.png"))
    end
    table = pretty_table(
        vcat(table_rows...);
        header = [
        ...
        body_hlines = collect(n_tracers:n_tracers:(length(table_rows) - 1)),
